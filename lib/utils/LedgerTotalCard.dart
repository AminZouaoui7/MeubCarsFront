...
import 'package:meubcars/Data/Models/paiment.dart'; // PaiementsApi, PaymentType, PaiementVM
...
class _LedgerTotalCardState extends State<_LedgerTotalCard> {
  final _fmt = NumberFormat.currency(locale: 'fr_FR', symbol: 'TND', decimalDigits: 3);
  ...
  Future<double> _fetchTotal() async {
    ...
    final list = await api.history(
      ...
      from: first,
      to: nextMonth, // < to : exclu => couvre le mois
    );

    final filtered = list.where(widget.include);
    final total = filtered.fold<double>(0.0, (s, p) => s + p.montant);
    return total;
  }
                  ...
                  if (snap.hasError) {
                    return Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Expanded(child: Text('Erreur: ${snap.error}', style: const TextStyle(color: Colors.redAccent))),
                        TextButton(onPressed: () => setState(() => _future = _fetchTotal()), child: const Text('Réessayer')),
                      ],
                    );
                  }
                  final v = snap.data ?? 0.0;
                  return Text(_fmt.format(v),
...
// 2) Administratif = Assurance + Carte grise + Vignette + Visite technique
class TotalAdministratifCard extends StatelessWidget {
  final int? voitureId;
  final DateTime? month;
  final VoidCallback? onTap;
  const TotalAdministratifCard({super.key, this.voitureId, this.month, this.onTap});
  ...
  Widget build(BuildContext context) {
    return _LedgerTotalCard(
      ...
      month: month,
      onTap: onTap,
      include: (p) => p.type == PaymentType.Assurance
...
// 3) Entretiens = PaymentType.Entretien (subType: 1=Vidange, 2=Panne)
class TotalEntretiensCard extends StatelessWidget {
  final int? voitureId;
  final DateTime? month;
  final VoidCallback? onTap;
  const TotalEntretiensCard({super.key, this.voitureId, this.month, this.onTap});

  @override
  Widget build(BuildContext context) {
    return _LedgerTotalCard(
      title: 'Total entretiens (vidange/panne)',
      icon: Icons.build, // ou Icons.build_circle_outlined
      voitureId: voitureId,
      month: month,
      onTap: onTap,
