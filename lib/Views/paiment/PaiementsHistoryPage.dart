...
class _PaiementsHistoryPageState extends State<PaiementsHistoryPage> {
  late PaiementsApi _api;
  Future<List<PaiementVM>>? _future;
  late final Dio _dio = Dio(BaseOptions(baseUrl: EndPoint.baseUrl));
  ...
  final _fmtNoSym = NumberFormat.currency(locale: 'fr_FR', symbol: '', decimalDigits: 3);
  ...
  Map<String, Map<PaymentType, double>> _buildMonthlyPivot(List<PaiementVM> rows) {
    final map = <String, Map<PaymentType, double>>{};
    for (final p in rows) {
      final key = '${p.datePaiement.year}-${p.datePaiement.month.toString().padLeft(2, '0')}';
      final row = map.putIfAbsent(key, () => {for (final t in _allTypesOrder) t: 0.0});
      row[p.type] = (row[p.type] ?? 0) + p.montant;
    }
    return map;
  }

  Map<int, Map<String, Map<PaymentType, double>>> _buildPivotByCar(List<PaiementVM> rows) {
    final map = <int, Map<String, Map<PaymentType, double>>>{};
    for (final p in rows) {
      final carMap = map.putIfAbsent(p.voitureId, () => {});
      final key = '${p.datePaiement.year}-${p.datePaiement.month.toString().padLeft(2, '0')}';
      final month = carMap.putIfAbsent(key, () => {for (final t in _allTypesOrder) t: 0.0});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      drawer: AppSideMenu(activeRoute: _activeRoute, onNavigate: _go, sections: _sections),
      body: Stack(
        fit: StackFit.expand,
        children: [
                  ...
                  child: Padding(
                    ...
                    child: FutureBuilder<List<PaiementVM>>(
                      ...
                      builder: (ctx, snap) {
                        ...
                        if (snap.hasError) {
                          return Center(child: Text('Erreur: ${snap.error}', style: TextStyle(color: Colors.red)));
                        }
                        final data = snap.data ?? [];
                        if (data.isEmpty) return const Center(child: Text('Aucun paiement.'));

                        final voitureIds = data.map((p) => p.voitureId).toSet();
                        _ensureMatricules(voitureIds);

                        final pivot = _buildMonthlyPivot(data);
                        final monthKeys = pivot.keys.toList()..sort((a, b) => b.compareTo(a));
                        ...
                        final typesNoAutre = _allTypesOrder.where((t) => t != PaymentType.Autre).toList();
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () => _exportGlobalPdf(pivot, _allTypesOrder),
                                  ),
                                ],
                              ),
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    icon: const Icon(Icons.picture_as_pdf),
                                    tooltip: 'Exporter en PDF',
                                    onPressed: () => _exportAllCarsPdf(byCar, typesNoAutre),
                                  ),
                                ],
                              ),
  ...
  // 📄 Export PDF global
  // 📄 Export PDF Global avec logo agrandi
...
// 📄 Export PDF Toutes Voitures avec logo agrandi
      ...
      pw.MultiPage(
        ...
        header: (ctx) => pw.Column(
          children: [
            ...
            pw.SizedBox(height: 8),
            pw.Text(
              'Tableau Financier - Toutes Voitures',
              style: pw.TextStyle(
                fontSize: 20,
                fontWeight: pw.FontWeight.bold,
                color: PdfColors.brown800,
