import 'dart:convert';
import 'dart:io' show File, HttpHeaders; // File uniquement pour mobile/desktop
...
PaymentType paymentTypeFromAny(dynamic raw, {String? sourceTable}) {
  if (raw is num) {
    final v = raw.toInt();
    switch (v) {
      case 0: return PaymentType.Assurance;
      case 1: return PaymentType.CarteGrise;
      case 2: return PaymentType.Vignette;
      case 3: return PaymentType.VisiteTechnique;
      case 4: return PaymentType.Entretien;
      case kBackendEnumTaxe: return PaymentType.Taxe;
      case kBackendEnumAutre: return PaymentType.Autre;
      default:
  ...
  if (raw != null) {
    ...
    switch (k) {
      case 'assurance': return PaymentType.Assurance;
      case 'cartegrise': return PaymentType.CarteGrise;
      case 'vignette': return PaymentType.Vignette;
      case 'visitetechnique':
      case 'visitetech':
      case 'controletechnique':
      case 'contrôletechnique':
        return PaymentType.VisiteTechnique;
      case 'entretien': return PaymentType.Entretien;
      case 'taxe':
      case 'taxes':
      case 'tax':
...
String paymentTypeToString(PaymentType t) {
  switch (t) {
    case PaymentType.Assurance: return 'Assurance';
    case PaymentType.CarteGrise: return 'CarteGrise';
    case PaymentType.Vignette: return 'Vignette';
    case PaymentType.VisiteTechnique: return 'VisiteTechnique';
    case PaymentType.Entretien: return 'Entretien';
    case PaymentType.Taxe: return 'Taxe';
    case PaymentType.Autre: return 'Autre';
  }
}
...
String paymentTypeToLabel(PaymentType t, {int? subType}) {
  switch (t) {
    case PaymentType.Assurance: return 'Assurance';
    case PaymentType.CarteGrise: return 'Carte grise';
    case PaymentType.Vignette: return 'Vignette';
    case PaymentType.VisiteTechnique: return 'Visite technique';
    case PaymentType.Entretien:
      if (subType == 1) return 'Entretien — Vidange';
      if (subType == 2) return 'Entretien — Panne';
      return 'Entretien';
    case PaymentType.Taxe: return 'Taxe';
    case PaymentType.Autre: return 'Paiement';
  }
}
...
class PaymentItem {
  ...
  factory PaymentItem.fromJson(Map<String, dynamic> j) {
    ...
    return PaymentItem(
      ...
      estActive: j['estActive'] as bool? ?? true, // ✅ défaut = true
    );
  }
}
...
class PaymentSummary {
  ...
  factory PaymentSummary.fromJson(Map<String, dynamic> j) {
    ...
    // ✅ filtrage des inactifs ici
    final allItems =
    itemsJson.map((e) => PaymentItem.fromJson(e as Map<String, dynamic>))
        .where((it) => it.estActive) // <= garde seulement actifs
        .toList();
...
class PaiementsApi {
  ...
  static Future<PaiementsApi> authed() async {
    ...
    final dio = Dio(
      BaseOptions(
        baseUrl: EndPoint.baseUrl, // ⚠️ doit déjà contenir /api si nécessaire
  ...
  Future<String?> uploadJustificatif({
    ...
    final endpoint = '/voitures/$voitureId/pieces-jointes/upload'; // ✅ pas de double /api
    final fullUrl = "${_dio.options.baseUrl}$endpoint";

    print("====== UPLOAD DEBUG (MOBILE) ======");
    print("📂 Fichier: $fileName");
    print("📦 BaseUrl: ${_dio.options.baseUrl}");
    print("📌 Endpoint: $endpoint");
    print("➡️ URL finale: $fullUrl");
    print("===================================");
...
extension PaiementsPayer on PaiementsApi {
  String _payerPathFor(PaymentType t) {
    switch (t) {
      case PaymentType.Assurance: return '/Assurances';
      case PaymentType.CarteGrise: return '/CartesGrises';
      case PaymentType.Vignette: return '/Vignettes';
      case PaymentType.VisiteTechnique: return '/VisitesTechniques';
      case PaymentType.Entretien: return '/Entretiens';
      case PaymentType.Taxe: return '/Taxes';
      case PaymentType.Autre: return '/Autres';
    }
  }
  ...
  Future<void> payerDocument({
    ...
    final path = _payerPathFor(type);
    final endpoint = "$path/$id/payer";
    final fullUrl = "${_dio.options.baseUrl}$endpoint";
